...
t_input[:,0] = np.ones(np.shape(time*60+172800))*73
t_input[:,1] = time
...
for pp in range(len(prns)):
    ...
    for rr in range(len(time)):
        ...
        enu_table.append([int(prns[pp]), np.degrees(az_), np.degrees(el_),range_]) # Append values to a list of lists

        # 89no COORDS
        [az_, el_, range_] = compute_azelrange(np.asarray(no89ECEF), np.asarray(satECEF))
        no89_table.append([int(prns[pp]), np.degrees(az_), np.degrees(el_),range_]) # Append values to a list of lists 
