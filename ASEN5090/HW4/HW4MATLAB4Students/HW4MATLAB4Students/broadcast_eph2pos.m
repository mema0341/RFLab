function [health,x] = broadcast_eph2pos(ephem_all,t_input,prn)
...
%               And also Melissa Mantey 10/5/2020
...
for tt = 1:sz % loop through all input times
    ...
    jj_test = max( find(t_input(tt) >= dt_ephem) ); % sat_ephem(:,17) = toe (sec into GPS week) of each entry
    ...
    % Logic to Decide if we are within two hours
    % Check Time to Create Logic to Decide if we are within two hours
    toe = sat_ephem(jj,17);            % time of ephemeris
    tj = t_input(:,2) - toe;
    %     if tj(jj) >=0 & tj(jj) <=7200    % We only care when tj is between 0 and 7200 seconds (2 hours)
    
    % Pull out common variables from the ephemeris matrix
    %======================================================================
    % UPDATE DT
    %         dt  = dt_input(tt) - dt_ephem(jj); % seconds difference from epoch
    dt  = t_input(tt,2) - toe; % seconds difference from epoch
    ...
    u = u + delta_uj;
    r = r + delta_rj;
    inc = inc + delta_ij + i_dot*dt; % SHAME
