...
def get_usn8ECEF():
	with open("USN8_location.txt", 'r') as file:
		pos = file.readline()
	strings = pos.split()
	usn8_location_txt_data = np.array([float(part) for part in strings])
	return usn8_location_txt_data.transpose()
...
def broadcast2posM(ephem_all, t_input, prn):
	...
	if sat_ephem:
		...
		#     jj = max( find(dt_input(tt) >= dt_ephem) ); # sat_ephem(:,17) = toe (sec into GPS week) of each entry
			...
			min_t_diff = np.abs( dt_input[tt]+0.001 - dt_ephem ).min()
			jj = np.where(np.abs(dt_input[tt]+0.001 - dt_ephem) == min_t_diff) #index of minimum
...
def ExpectedRange(sat,station, Tr, almanac, prn_val):
	...
	rot_posHere = np.dot(rot,posHere[0,:])
	rangeNew = np.linalg.norm(rot_posHere - station)
	azelrange_vals = np.zeros((1,3))
	changed = np.zeros((3))
	while abs(rangeNew - Geo)> 1e-6:
		...
		azelrange_vals = azelrange(station, np.asarray([changed]))
		rangeNew = np.linalg.norm(changed - station)
	return rangeNew,  azelrange_vals[0,0],  azelrange_vals[0,1], changed, clock_corr, rel_corr
