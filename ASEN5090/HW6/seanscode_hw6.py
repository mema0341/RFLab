...
timevec = np.arange(172830, 172830+86401, 31)
...
base = np.datetime64("2020-09-01T00:00:30")
#Washington ECEF estimate
Washington = llatoECEF(38.889, -77.049, 0)
#defining_matrixsome things
i = 0
ExRa = []
PR = []
Clock = []
...
for Sat in obs_data:
	if obs_data[Sat].time[0] == base:
		...
		iofree, iocorr = Ionofree(np.array(obs_data[Sat].signals["L1"].pr[1:3]), L1f, obs_data[Sat].signals["L2"].pr[1:3], L2f)
		ER, cc, relativity, azimuth,elevation, sat_Vec = ExpectedRange(vec1_ECEF, 172830,a, int(Sat[1:])) #grabbing_matrixthe integer PRN from the Satellite GXX string
		...
		print("Azimuth: {} deg_matrix  Elevation: {} deg".format(azimuth, elevation))
...
# 			iofree, iocorr = Ionofree(np.array(obs_data[Sat].signals["L1"].pr[1:3]), L1f, obs_data[Sat].signals["L2"].pr[1:3], L2f)
# 			ER, cc, relativity, azimuth,elevation, sat_Vec = ExpectedRange(newr, timeN,a, int(Sat[1:])) #grabbing_matrixthe integer PRN from the Satellite GXX string
# 			tropoCorr =  TopoCorrect(elevation, 2)
			
# 			#a new row for the g_matrixmatrix
# 			row = [-(sat_Vec[0]- vec1_ECEF[0])/ ER, -(sat_Vec[1]- vec1_ECEF[1])/ ER, -(sat_Vec[2]- vec1_ECEF[2])/ ER, 1]
# 			g_matrix= np.append(g_matrix, np.array([row]), axis = 0)
# 			#The Residual, we'll keep trying_matrixto change ER and time 
# 			dy = iofree[0] - (ER - cc -relativity + tropoCorr )
# 			Resid = np.append(Resid, dy)
# 	#do least squares and continue
# 	delt, _, _, _ = np.linalg.lstsq(g_matrix, Resid, rcond = None)
# 	newr = newr + delt[0:3]
